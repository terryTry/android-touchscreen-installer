...
面向安装、调试与售后人员的 Windows x64 ADB 图形化工具。客户端内置 Windows 版
...
```bash
# 首次准备浏览器（CI/Linux 可使用 npx playwright install --with-deps chromium）
...
npm run test:e2e -- --grep '签名冲突'

# 查看最近一次 HTML 报告
npm run test:e2e:report
```

用例位于 `tests/e2e/`。Playwright runner 自动启动和停止独立的 Vite 服务
（`127.0.0.1:4173`，端口占用时明确失败），在 Chromium 中加载生产 React 页面、
CSS 和真实 xterm。每条用例使用独立浏览器上下文，在页面初始化前注入类型约束的
`window.adbTool` 测试实现，记录请求并提供显式响应与快照/终端事件。
测试不会启动 Electron 主进程、ADB Server 或操作真实设备。

默认窗口 `1180×820` 和最小窗口 `980×680` 都执行同一套场景：

- USB 无设备、未授权、离线、权限不足、多设备选择与断开；TCP 表单校验和连接修复。
- 设备应用搜索、包名读取、前台/桌面入口，APK 选择与拖放。
- 安装预检、调试降级、操作进度、忙碌互斥、应用控制与 Launcher 命令。
- 卸载、签名冲突重装、Root、系统/特权部署与托管回滚的确认约束。
- 操作记录筛选、详情加载、诊断展开与复制请求。
- 终端快捷输入、参数预览、输出渲染、复制粘贴、历史、Shell 会话和中断。
- 两种窗口尺寸下的入口可达性；主界面截图附在报告中。

失败时保存截图和 trace 到 `test-results/`，HTML 报告位于 `playwright-report/`；
两者均不提交到 Git。默认不重试，避免掩盖不稳定用例。可用
`npx playwright show-trace <trace.zip路径>` 查看失败步骤。

这套验收覆盖渲染进程交互和 Preload API 调用契约。测试响应模拟的安装成功、
Root、重连、复制等结果不代表真实设备或系统能力已验证。Electron 原生菜单/文件
选择器、IPC、系统剪贴板、APK 解析、真实 ADB 与 Windows 安装/驱动仍需分别验证；
已有主进程单元测试继续通过 `npm run build` 运行。

## 局域网设备搜索

在 TCP/IP 模式点击“搜索局域网设备”，无需填写端口，立即使用第一张可用 IPv4 网卡自动发现 mDNS ADB 服务，同时探测默认 TCP 端口 5555。需要调整范围时，选择“电脑使用的网络”；设备使用自定义端口且未被自动发现时，展开“高级选项”修改“扫描端口”，再点击“开始搜索”。扫描端口仅影响 TCP 探测，mDNS 仍会发现并使用设备公告的其他端口，不进行全端口扫描。收起高级选项保留当前扫描端口，重新点击“搜索局域网设备”则恢复第一张网卡和默认端口。程序合并所选 IPv4 子网内的 mDNS ADB 服务和端口开放候选；在设备卡片点击“连接此设备”，直接发起 ADB 连接；卡片会显示连接中、已连接或失败原因，并支持原位重试。

端口开放不代表目标一定是安卓屏。无线调试配对服务不能直接作为连接端口；首次配对请在外部终端完成，再搜索其调试服务。未开启网络 ADB、网络隔离或防火墙阻断时可能搜索不到设备，可继续手动填写 IP 和端口。

每次最多扫描 4094 个地址，并发 32，单地址超时 450 毫秒；超过范围会明确提示，不会静默截断。取消后不再探测新地址，等待当前探测及 mDNS 命令结束（mDNS 最长约 4 秒）。当前自动发现仅支持 IPv4。

搜索结果按网卡、IPv4 地址/子网掩码和端口缓存在本次运行的内存中，不写磁盘。切回有缓存的条件时显示上次发现时间，并自动重新扫描；验证完成前历史候选不可连接，取消或失败后保留历史标记，成功扫描则替换旧列表。切换搜索条件不影响已建立的设备连接。
